...
export const vehicleUpdateSchema = vehicleCreateSchema
  ...
  .extend({
    ...
    vatStatus: z
      .enum(["excluding_vat", "including_vat", "margin_scheme", "no_vat"])
      .optional(),
    capId: nullableText(80),
    plate: nullableText(40),
    interiorColour: nullableText(80),
    interiorMaterial: nullableText(120),
    fuelConsumptionUrbanMpg: z.coerce.number().min(0).max(1000).optional().nullable(),
    fuelConsumptionExtraUrbanMpg: z.coerce.number().min(0).max(1000).optional().nullable(),
    fuelConsumptionCombinedMpg: z.coerce.number().min(0).max(1000).optional().nullable(),
    euroEmissionsStandard: nullableText(40),
    insuranceGroup: nullableText(40),
    roadTaxAnnual: z.coerce.number().min(0).max(100_000).optional().nullable(),
    ulezStatus: z.enum(["compliant", "non_compliant", "unknown"]).optional().nullable(),
    wheelchairAccessible: z.boolean().optional().nullable(),
    acceleration060Seconds: z.coerce.number().min(0).max(1000).optional().nullable(),
    topSpeedMph: z.coerce.number().int().min(0).max(1000).optional().nullable(),
    torqueLbFt: z.coerce.number().int().min(0).max(10_000).optional().nullable(),
    aspiration: nullableText(80),
    engineLocation: nullableText(80),
    engineNumber: nullableText(120),
    chassisNumber: nullableText(120),
    cylinderCount: z.coerce.number().int().min(1).max(32).optional().nullable(),
    gearCount: z.coerce.number().int().min(1).max(20).optional().nullable(),
    driveType: nullableText(80),
    grossWeightKg: z.coerce.number().int().min(0).max(100_000).optional().nullable(),
    ...
    keeperStartDate: z.iso.date().optional().nullable(),
    acquiredAt: z.iso.date().optional().nullable(),
    advertisedCondition: z
      .enum(["new", "used", "demonstrator", "pre_registered"])
      .optional(),
    insuranceWriteOffCategory: nullableText(40),
    serviceHistoryVisible: z.boolean().optional(),
    serviceHistorySummary: nullableText(1000),
    silentSalesmanHeadline: nullableText(180),
    silentSalesmanSummary: nullableText(4000),
    silentSalesmanCallToAction: nullableText(240),
